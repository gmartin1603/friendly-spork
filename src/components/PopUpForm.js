...
function PopUpForm({ dept }) {
  ...
  const fillPost = () => {
    ...
    if (formObj.norm) {
      setState((prev) => ({
        ...
        color: formObj.color,
        tag: {
          name: formObj.tag.name,
          reason: formObj.tag.reason,
          color: formObj.color,
        },
        shift: formObj.shift,
        seg: formObj.seg,
  ...
  const handleClick = (e) => {
    ...
    if (state.seg[e.target.value]) {
      ...
      obj = {
        ...state.seg,
        [e.target.value]: { name: "", forced: false, trade: false },
      };
    }
  ...
  const handleChange = (e) => {
    ...
    switch (e.target.name) {
        ...
        if (e.target.value) {
          ...
          if (num < state.date + 11 * 60 * 60 * 1000) {
            ...
            dispatch({
              ...
              load: {
                type: -1,
                message: `Down Date updated to ${new Date(newDown)
                  .toDateString()
                  .slice(
                    3,
                    15
      ...
      case "creator":
        let value = "";
        if (e.target.value) {
          value = `${e.target.value[0].toUpperCase()}${e.target.value.slice(
            1
          )}`;
        }
        ...
        break;
      default:
        e.target.name ? console.log(e.target.name) : console.log("No Name");
    }
  };
    ...
    <div className={styles.backDrop}>
      {
        ...
        >
            ...
            <>
              {sel ? (
                <div className={`font-bold`}>
                  ...
                  {profile.level > 1 && (
                    <FormInput
                      ...
                      value={state.editor}
                      valiTag={
                        state.editor.length < 3 ? "*Required" : undefined
                      }
                      type="text"
                      name="creator"
                      label="Your Name"
                ...
                <div className={`w-full font-bold text-xl`}>
                  <button className={styles.fullSeg} onClick={(e) => fill(e)}>
                    Fill
                  </button>
                </div>
              )}
              ...
              <>
                {state.shift.id && (
                  <FormInputCont
                    ...
                    label="Hours to Fill"
                    valiTag={
                      Object.keys(state.seg).length === 0
                        ? "*Required"
                        : undefined
                    }
                  >
          ...
          <div className={`h-50 w-full flex justify-around mt-35`}>
            {formObj.modify && profile.level < 2 && (
              <button
