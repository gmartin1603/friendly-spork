...
import { Cancel, CheckBox, Delete, DeleteOutlineOutlined, Edit } from "@mui/icons-material";
...
function Row(props) {
	...
	const deleteUser = (user) => {
		...
		toast.promise(usersDashoardService.deleteUser({ uid: user.id }), {
  ...
  const disableUser = (disabled) => {
    ...
    .then(res => {
      ...
      pending: `${disabled ? 'Disabling' : 'Enabling'} user...`,
      success: `User successfully ${disabled ? 'disabled' : 'enabled'}!`,
      error: `Error ${disabled ? 'disabling' : 'enabling'} user`,
    })
  };

  React.useEffect(() => {
    // if (row.displayName === 'CASC EE') {
    //   row.disabled = true
    //   console.log('Row', row)
    // }
  }, [row])
			...
			<TableRow>
				<TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
					<Collapse in={open} timeout="auto" unmountOnExit>
						<Grid className="flex justify-around" container spacing={2}>
                  ...
                  <Tooltip title="Delete User" placement="top">
                    ...
                    >
                      <Delete />
                          ...
                          {jobsOpen && row.details.length > 3 && (
                              row.details.slice(3).map((detailRow, i) => {
                                return (
                                  <TableRow key={detailRow.id}>
                                    ...
                                    <TableCell component="td">{detailRow.date}</TableCell>
                                  </TableRow>
                                );
                              })
                            )}
			...
			>
				<Box sx={style.deleteUser}>
					<Typography variant="h6">
						...
						<b> {`${row.name.first} ${row.name.last}`} </b>?
					</Typography>
					...
					<TableHead>
						<TableRow>
							<TableCell sx={{ bgcolor: "black" }}>
								<Button
									data-cy="dashboard-add-user-button"
									color="success"
									size="small"
									variant="contained"
