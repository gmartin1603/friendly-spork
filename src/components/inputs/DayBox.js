...
const DayBox = ({
		...
		if (state[day].slots > 1) {
			...
			state[day].seg.one.segs.map((slot, i) => {
				if (!shifts[state.shift].segs.three) {
					...
					if (
						!slot.name &&
						!state[day].seg.two.segs[i].name &&
						!state[day].seg.three.segs[i].name
					) {
						// validated = false
						arr.push(i);
					}
				...
				if (slots > 1) {
					...
					Object.keys(shifts[state.shift].segs).map((key) => {
						if (key !== "full") {
							...
							for (let i = 0; i < slots; i++) {
								...
								obj[key].segs = [
									...obj[key].segs,
									{ name: downRef, forced: false, trade: false },
								];
							}
						}
					});
					...
					Object.keys(shifts[state.shift].segs).map((key) => {
						if (key !== "full") {
							obj[key] = {
								key: key,
								label: shifts[state.shift].segs[key],
								name: downRef,
								forced: false,
								trade: false,
								bids: [],
	...
	const styles = {
		main: ` bg-${
			disabled ? "gray" : color
		} border min-w-[180px] w-min h-max px-.01 py-.02 m-.01`,
		showBtn: [`${show ? button.red : button.green} `],
		selBtn: `${button.green} w-[25%]`,
		...
		selected: `${button.green} shadow-clearBlack shadow-sm rounded border-2 border-clearBlack text-center text-base`,
		segBtn: `${button.green}`,
	...
	return (
		<div className={styles.main}>
			...
			<h6 className={`font-bold text-lg text-white mb-.05`}>
				{new Date(label).toDateString().slice(0, 3)}{" "}
				{new Date(label).toDateString().slice(3, 10)}
			</h6>
			{!disabled && (
				<Button
					name="showTog"
			...
			{!disabled && state[day].id && (
				<div>
					<FormInputCont
						...
						label="Hours to Fill"
						valiTag={
							Object.keys(state[day].seg).length === 0
								? "*Min 1 Segment Required"
								: undefined
						}
					>
						{slots > 1 ? (
							...
							state[day].seg.one.segs.map((slot, i) => (
								<div className={`flex flex-col p-10 justify-center`} key={i}>
									...
									<div className={`flex`}>
										<button
											className={`${
												state[day].seg.one.segs[i].name
													? styles.selected
													: styles.check
											} ${errors.includes(i) && "border-2 border-red"} my-10`}
											value="one"
											id={i}
											key={`one${i}`}
							...
							<div className={`flex justify-around text-center`}>
								<button
									className={
										state[day].seg.one ? styles.selected : styles.check
									}
									value="one"
									id={-1}
									onClick={(e) => handleClick(e)}
								...
								<button
									className={
										state[day].seg.two ? styles.selected : styles.check
									}
									value="two"
									id={-1}
									onClick={(e) => handleClick(e)}
								...
								{shifts[state.shift].segs.three && (
									<button
										className={
											state[day].seg.three ? styles.selected : styles.check
										}
										value="three"
										id={-1}
										onClick={(e) => handleClick(e)}
