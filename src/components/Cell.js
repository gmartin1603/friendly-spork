...
function Cell(props) {
  ...
  const [{ profile, shifts, rota }, dispatch] = useAuthState();
  ...
  return (
    <td
      id={props.id}
      align={props.align}
      className={`hover:text-[gray] hover:font-bold ${
        props.hoverTog ? "font-extrabold" : ""
      }`}
      style={
        props.disabled
          ? { backgroundColor: color, cursor: "default" }
          : { backgroundColor: color, cursor: "pointer" }
      }
      onClick={() =>
        props.hoverTog
          ? openForm()
          : dispatch({ type: "ARR-PUSH", load: props.id, name: "scale" })
      } //returns cell info
    >
      {props.post ? styleValue() : props.value}
    </td>
  );
}
