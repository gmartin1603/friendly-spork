...
function Post({ job, post, shift, label }) {
  ...
  const styles = {
    ...
    } border-b-4 border-clearBlack bg-green rounded-t-xl text-center`,
    ...
    <div className={styles.main}>
      ...
      >
        ...
        <p className={styles.p}>
          Down:{" "}
          {`${new Date(post.down).getMonth() + 1}/${new Date(
            post.down
          ).getDate()} @ ${new Date(post.down).toLocaleTimeString()}`}
        </p>
        {/* <p className="font-semibold">{!disabled && "Click here to sign"}</p> */}
        {post.slots > 1 && (
          <p className={`${styles.p} font-bold bg-clearBlack`}>
            {" "}
            {`X ${post.slots}`}{" "}
          </p>
        )}
      ...
      <div className={styles.listContainer}>
        {post.seg.one ? (
          <BidList post={post} seg="one" shift={shift} profile={profile} />
        ) : null}
        {post.seg.two ? (
          <BidList post={post} seg="two" shift={shift} profile={profile} />
        ) : null}
        {post.seg.three ? (
          <BidList post={post} seg="three" shift={shift} profile={profile} />
        ) : null}
      </div>
      <div className={styles.foot}>
        <p className={styles.p}>
          {post.lastMod
            ? `Last Update by: ${post.lastMod}`
            : `Posted By: ${post.creator}`}
        </p>

        <p className={styles.p}>
          {post.lastMod
            ? `On: ${new Date(post.modDate)
                .toDateString()
                .slice(4, 10)} @ ${new Date(post.modDate).toLocaleTimeString()}`
            : `On: ${new Date(post.created)
                .toDateString()
                .slice(4, 10)} @ ${new Date(
                post.created
              ).toLocaleTimeString()}`}
      ...
      {post.filled ? (
        post.slots > 1 ? (
          post.seg[seg].segs.map((slot, i) => (
            <p key={`one${i}`} className={styles.userBid}>
              {slot.name}
            </p>
          ))
        ) : (
          ...
          {post.seg[seg].bids?.length > 0 ? (
            post.seg[seg].bids.map((bid) => (
              <li
                key={bid.name}
                className={
                  !post.filled && bid.name === profile.dName
                    ? styles.userBid
                    : ""
                }
              >
