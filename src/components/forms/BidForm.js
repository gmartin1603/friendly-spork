  ...
  return (
    <div className={styles.backDrop}>
      <form className={styles.form} action="Bid">
        ...
        {errors.length > 0 &&
          errors.map((error) => (
            <p className={`bg-red text-black text-center p-.01`}>
              {error.message}
            </p>
          ))}
        <FormInput label="Position" value={`${formObj.title}`} disabled />
        <FormInput
          label="Posting Date:"
        ...
        />
        <div>
          <h1 className={`w-full text-center`}>Segment Selection</h1>
              ...
              {notes === "text" && (
                ...
                >
                  <textarea
                    ...
                    value={area}
                  />
                  <p className={`text-black text-sm w-full text-right`}>
                    {area.length}/160
                  </p>
                </FormInputCont>
              )}
        ...
        >
          {mod
            ? `Save Changes`
            : `Submit ${selections.length > 1 ? "Signatures" : "Signature"}`}
        </button>
        {mod && (
          <button
...
function SigBtn({ post, seg, shift, selections, preview, handleClick }) {
  ...
  return (
    <div className={styles.segCont}>
      ...
      <ol>
        {preview[seg] &&
          preview[seg].map((bid) => <li key={bid.name}> {bid.name} </li>)}
      </ol>
    </div>
  );
}
