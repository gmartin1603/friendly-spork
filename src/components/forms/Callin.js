...
function CallIn({}) {
	...
	const [{ formObj, users, view, options, filtered, shifts }, dispatch] =
		useAuthState();
	...
	useEffect(() => {
		...
		for (const key in templ) {
			...
			if (key === "full") {
				if (segSel.length > 1) {
					arr.push({
						value: `All ${segSel.length * 4} Hours`,
						filled: false,
						seg: key,
					});
				}
	...
	return (
		<div style={styles.main}>
			<div style={styles.form}>
				...
				{fill ? (
					<FillForm state={state} setState={setState} />
				) : (
					<>
						{step === 1 && (
							<>
								...
								<label htmlFor="job">
									...
									<input
										...
										type="text"
										style={styles.field}
										defaultValue={`${formObj.pos.load.label} - ${
											formObj.shift.label
										} Shift`}
										disabled
									/>
				...
				<div style={styles.btnCont}>
						...
						<>
							{step > 1 && (
								<button
									style={{
										...styles.submit,
										backgroundColor: disablePrev ? "grey" : "green",
										cursor: disablePrev ? "none" : "pointer",
									}}
					...
					<button style={styles.cancel} onClick={(e) => closeForm(e)}>
						Cancel
					</button>
				</div>
			</div>
